    ...
    for stockid in stocknum:
        ...
        for Y in range(
            ...
            for i, K in enumerate(
                    df.iloc[Y + X_window:Y + X_window + K_changedays, 6]):
        ...
        se_rcd = pd.DataFrame(
            recordchangedata_np_ar, columns=['plus', 'minus'])
        se_rcd.to_hdf(
            h5datapath + stockid + '_table.h5',
            'stock_data_table',
            format='table',
            mode='w')
...
def stock_tablemake(stocknum, h5datapath):
    ...
    for stockid in stocknum:
        ...
        table_sumchange_df.to_hdf(
            h5datapath + stockid + '_table_sumchange.h5',
            'stock_data_table',
            format='table',
            mode='w')
    ########################################       求證總數無誤  將所有變化量統合
    ...
    for stockid in stocknum:
        df_table = pd.read_hdf(h5datapath + stockid + '_table_sumchange.h5',
                               'stock_data_table')
        sum_df = pd.concat([sum_df, df_table], ignore_index=True)
...
def observe_relation(stockid, h5datapath):
    ######################################         觀察變化量與價格關係
    df_table = pd.read_hdf(h5datapath + str(stockid) + '_table.h5',
                           'stock_data_table')
