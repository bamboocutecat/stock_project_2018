...
def drawpic(stockid, X_window=50, Y_slicing=1, K_changedays=50, pic_check=1,
    ...
    for X_pics in range(0, int((len(df) - X_window) / Y_slicing + 1 - K_changedays)):
        ...
        if X_pics < 301:
            ax5.set(ylim=[df.iloc[1:301, 6].quantile(
                0.1), df.iloc[1:301, 6].quantile(0.9)])
        else:
            ax5.set(ylim=[df.iloc[X_pics-300:X_pics, 6].quantile(0.1),
                          df.iloc[X_pics-300:X_pics, 6].quantile(0.9)])

        ax5.fill_between(range(
        ...
        if X_pics < 301:
            ax6.set(ylim=[df.iloc[1:301, 2].quantile(
                0.1), df.iloc[1:301, 2].quantile(0.9)])
        else:
            ax6.set(ylim=[df.iloc[X_pics-300:X_pics, 2].quantile(0.1),
                          df.iloc[X_pics-300:X_pics, 2].quantile(0.9)])
