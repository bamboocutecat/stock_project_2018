...
def drawpic(
    ...
    ax = plt.subplot2grid(
        (20, 4), (0, 0), rowspan=7, colspan=4, facecolor='#07000d')
    ...
    for X_pics in range(0, int((len(df) - X_window) / Y_slicing + 1)):
        ...
        candlestick2_ochl(
            ...
            alpha=0.6)
        ax.plot(
            range(0, 50), sma_10[X_pics:X_pics + 50], color='#ffff00', lw=5)
        ax.plot(
            range(0, 50), sma_30[X_pics:X_pics + 50], color='#0066ff', lw=5)
        # try:
        ...
        ax2 = plt.subplot2grid(
            (20, 4), (7, 0), rowspan=2, colspan=4, facecolor='#07000d')
        fillcolor = '#c58fff'
        ...
        ax2.plot(
            df_slice['日期'], macd[X_pics:X_pics + 50], color='#4ee6fd', lw=5)
        ax2.plot(
            df_slice['日期'], ema9[X_pics:X_pics + 50], color='#ffc78f', lw=5)
        ax2.fill_between(
            df_slice['日期'], (macd - ema9)[X_pics:X_pics + 50],
            0,
            alpha=0.5,
            facecolor=fillcolor,
            edgecolor=fillcolor)
        ...
        ax3 = plt.subplot2grid(
            (20, 4), (9, 0), rowspan=2, colspan=4, facecolor='#07000d')
        rsi = rsiFunc(df['收盤價'])
        rsiCol = '#ff99ff'
        posCol = '#ffff00'
        negCol = '#ff3300'
        ...
        ax3.fill_between(
            df_slice['日期'],
            rsi[X_pics:X_pics + 50],
            70,
            where=(rsi[X_pics:X_pics + 50] >= 70),
            facecolor=negCol,
            edgecolor=negCol,
            alpha=0.5)
        ax3.fill_between(
        ...
        ax4.plot(
            df_slice['日期'], mom[X_pics:X_pics + 50], '#668cff', linewidth=5)

        # 價格關係   一年參考關係
        ax5 = plt.subplot2grid(
            (20, 4), (13, 0), rowspan=4, colspan=4, facecolor='#07000d')

        if X_pics < 301:
            ax5.set(ylim=[
                df.iloc[1:301, 6].quantile(0.1), df.iloc[1:301, 6].quantile(
                    0.9)
            ])
        else:
            ax5.set(ylim=[
                df.iloc[X_pics - 300:X_pics, 6].quantile(0.1), df.iloc[
                    X_pics - 300:X_pics, 6].quantile(0.9)
            ])
        ...
        ax6 = plt.subplot2grid(
            (20, 4), (17, 0), rowspan=4, colspan=4, facecolor='#07000d')

        if X_pics < 301:
            ax6.set(ylim=[
                df.iloc[1:301, 2].quantile(0.1), df.iloc[1:301, 2].quantile(
                    0.9)
            ])
        else:
            ax6.set(ylim=[
                df.iloc[X_pics - 300:X_pics, 2].quantile(0.1), df.iloc[
                    X_pics - 300:X_pics, 2].quantile(0.9)
            ])
        ...
        picdata = PIL.Image.open(pic_path + stockid + 'pic/' +
                                 str(X_pics).zfill(4) + '_' + stockid + '.jpg')
        picdata = picdata.resize((224, 224), Image.ANTIALIAS)
        picdata.save(pic_path + stockid + 'pic/' + str(X_pics).zfill(4) + '_' +
                     stockid + '.jpg')
        print(stockid + ' = ' + str(countpic))
